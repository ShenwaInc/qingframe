...
注：表名中的 `pre_` 表示数据表统一前缀，该前缀可以通过修改项目的 `.env` 文件来自定义，默认值为 `qing_` 。
...
## 9. pre_uni_account_extra_modules 表
...
## 16. pre_users_operate_history 表
...
## 24. pre_users_founder_own_users 表
...
## 28. pre_uni_account_modules_shortcut 表
...
## 31. pre_core_cache 表
**表用途说明：** 核心缓存表，用于存储系统的核心缓存数据，以键值对的形式保存缓存信息，提高系统性能。

| 字段名 | 字段类型 | 默认值 | 额外属性 | 注释 |
| --- | --- | --- | --- | --- |
| key | string(100) | - | primary | 缓存键名 |
| value | text | - | - | 缓存值（序列化数据） |


---

## 32. pre_core_paylog 表
...
## 47. pre_site_store_create_account 表
