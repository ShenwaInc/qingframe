...
function material_sync($material, $exist_material, $type) {
	...
	foreach ($material as $news) {
		...
		if (empty($material_exist)) {
			...
			if ($type == 'video') {
				$material_data['filename'] = $news['name'];
				$material_data['tag'] = serialize(array('title' => $news['name']));
			}
            $attachid = pdo_insert('wechat_attachment', $material_data, true);
		} else {
			if ($type == 'image') {
				$material_data = array(
					'createtime' => $news['update_time'],
			...
			if ($type == 'video') {
				$tag = empty($material_exist['tag']) ? array() : unserialize($material_exist['tag']);
				$material_data = array(
					'filename' => $news['name'],
					'createtime' => $news['update_time'],
					'tag' => serialize(array('title' => $news['name'], 'url' => $tag['url']))
				);
				pdo_update('wechat_attachment', $material_data, array('uniacid' => $_W['uniacid'], 'media_id' => $news['media_id']));
			}
...
function material_news_set($data, $attach_id) {
	...
	foreach ($data as $key => $news) {
		...
		$post_news[] = array(
			...
			'content' => trim(htmlspecialchars_decode($news['content'])),
			'url' => $news['url'],
	...
	if (!empty($attach_id)){
		...
		Cache::forget(CacheService::system_key('material_reply', array('attach_id' => $attach_id)));
	} else {
        ...
        $attach_id = pdo_insert('wechat_attachment', $wechat_attachment, true);
...
function material_get($attach_id) {
	...
	if (is_numeric($attach_id)) {
		$material = DB::table('wechat_attachment')->where('id',$attach_id)->first();
	} else {
		$media_id = trim($attach_id);
		$material = DB::table('wechat_attachment')->where('media_id',$media_id)->first();
	}
	if (!empty($material)) {
		if ($material['type'] == 'news') {
			$news = DB::table('wechat_news')->where('attach_id',$material['id'])->get()->toArray();
			if (!empty($news)) {
				foreach ($news as &$news_row) {
					$news_row['content_source_url'] = trim($news_row['content_source_url']);
...
function material_build_reply($attach_id) {
	...
	$cachekey = CacheService::system_key('material_reply', array('attach_id' => $attach_id));
