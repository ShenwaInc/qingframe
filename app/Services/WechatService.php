...
class WechatService extends WeAccount
{
    ...
    public function menuDelete($menuid = 0) {
        ...
        $result = @json_decode($response['content'], true);
        if (!empty($result['errcode'])) {
            return error(-1, "访问微信接口错误, 错误代码: {$result['errcode']}, 错误信息: {$result['errmsg']},错误码：{$result['errcode']}");
        }
    ...
    public function fansAll($startopenid = '') {
        ...
        if (empty($result)) {
            ...
            return error(-1, "访问公众平台接口失败, 错误: {$result['errmsg']}");
        }
        $return = array();
    ...
    public function getAccessToken() {
        ...
        if ('40164' == $token['errcode']) {
            return error(-1, $token['errmsg']);
        }
        if (empty($token) || !is_array($token) || empty($token['access_token']) || empty($token['expires_in'])) {
            return error('-1', '获取微信公众号授权失败！错误代码:' . $token['errcode']);
        }
        $record = array();
    ...
    public function clearAccessToken() {
        ...
        if (is_error($response) && '40001' == $response['errno']) {
            Cache::forget(CacheService::system_key('accesstoken', array('uniacid' => $this->account['uniacid'])));
        }
    ...
    public function getJsApiTicket() {
        $cachekey = CacheService::system_key('jsticket', array('uniacid' => $this->account['uniacid']));
    ...
    public function isTagSupported() {
        return (!empty($this->account['key']) &&
                !empty($this->account['secret']) || 3 == $this->account['type']) &&
    ...
    public function fansTagAdd($tagname) {
        ...
        $data = stripslashes(json_encode(array('tag' => array('name' => $tagname)), JSON_UNESCAPED_UNICODE));
        Cache::forget(CacheService::system_key('account_tags', array('uniacid' => $this->account['uniacid'])));
    ...
    public function fansTagFetchAll() {
        $cachekey = CacheService::system_key('account_tags', array('uniacid' => $this->account['uniacid']));
        $cache = Cache::get($cachekey);
        ...
        if (!is_error($tags)) {
            Cache::put($cachekey, array('tags' => $tags, 'expire' => TIMESTAMP + 3600), 3400);
        }
        return $tags;
    }
    ...
    public function fansTagEdit($tagid, $tagname) {
        ...
        $data = stripslashes(json_encode(array('tag' => array('id' => $tagid, 'name' => $tagname)), JSON_UNESCAPED_UNICODE));
    ...
    public function sendTplNotice($touser, $template_id, $postdata, $url = '', $topcolor = '#FF683F', $miniprogram = array('appid' => '', 'pagepath' => '')) {
        if (empty($this->account['key']) || 4 != $this->account['level']) {
    ...
    public function editMaterialNews($data) {
        ...
        $response = $this->requestApi($url, stripslashes(json_encode($data, JSON_UNESCAPED_UNICODE)));
        if (is_error($response)) {
            return $response;
        }
    ...
    public function uploadVideoFixed($title, $description, $path) {
        ...
        $data = array(
            'media' => '@' . $path,
            'description' => stripslashes(json_encode(array('title' => $title, 'introduction' => $description), JSON_UNESCAPED_UNICODE)),
        );
    ...
    public function addMatrialNews($data) {
        ...
        $data = stripslashes(urldecode(json_encode($data, JSON_UNESCAPED_UNICODE)));
        $response = $this->requestApi($url, $data);
        if (is_error($response)) {
            return $response;
        }
    ...
    public function getMaterial($media_id, $savefile = true) {
        ...
        if ($savefile && !empty($response['headers']['Content-disposition']) && strexists($response['headers']['Content-disposition'], 'filename=')) {
            ...
            $filename .= FileService::file_random_name($filename, $pathinfo['extension']);
            $fileservice = new FileService();
            $fileservice->file_write($filename, $response['content']);
            $fileservice->file_remote_upload($filename);
    ...
    public function getOauthAccessToken() {
        $cachekey = CacheService::system_key('oauthaccesstoken', array('acid' => $this->account['acid']));
        $cache = Cache::get($cachekey);
        if (!empty($cache) && !empty($cache['token'])) {
            return $cache['token'];
        }
    ...
    public function commentReply($msg_data_id, $user_comment_id, $content, $index = 0) {
        ...
        return $this->requestApi($url, stripslashes(json_encode($data, JSON_UNESCAPED_UNICODE)));
    }
    ...
    protected function requestApi($url, $post = '') {
        ...
        $result = @json_decode($response['content'], true);
        if (is_error($response)) {
            return error($result['errcode'], "访问公众平台接口失败, 错误码: {$result['errcode']}");
        }
        if (empty($result)) {
            return error(-1, "接口调用失败, 元数据: {$response['meta']}");
        } elseif (!empty($result['errcode'])) {
